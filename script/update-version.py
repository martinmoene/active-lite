#!/usr/bin/env python
#
# Copyright 2017-2021 by Martin Moene
#
...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+active_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( active_lite_version "{major}.{minor}.{patch}" )\n' )

    , ( 'include/nonstd/active.hpp'
        , r'\#define\s+active_lite_MAJOR\s+[0-9]+\s*$'
        , '#define active_lite_MAJOR  {major}' )

    , ( 'include/nonstd/active.hpp'
        , r'\#define\s+active_lite_MINOR\s+[0-9]+\s*$'
        , '#define active_lite_MINOR  {minor}' )

    , ( 'include/nonstd/active.hpp'
        , r'\#define\s+active_lite_PATCH\s+[0-9]+\s*$'
        , '#define active_lite_PATCH  {patch}\n' )
)
